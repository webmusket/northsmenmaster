...
class CartController extends Controller
{
    ...
    public function applyCoupon(Request $request){
        ...
        if($couponCount == 0){
            ...
            if ($couponDetails->individual == 1 && $couponDetails->couponemail !=  $user_email) {
                $coupondata['err'] = 'This coupon is not for you!';
                return $coupondata;
            }
            ...
            $pro = $couponDetails->couponproducts;
            $expro = $couponDetails->excludecouponproducts;


            if (count($pro) != 0) {
                $final_pro = array_diff($pro, $expro);
            }else{
                if (count($total_pro) > 0) {
                    $total = $total_pro;
                }else{
                    $total = App\Readymadeproduct::pluck('title');
                }
                
                $final_pro = array_diff($total, $expro);
            }
            $status = 0;

            foreach ($userCart as $item) {
                if(in_array($item, $final_pro)){
                    $status = 1;
                }
            }

            if ($status == 0) {
                $coupondata['err'] = 'This coupon is valid for these products';
                return $coupondata;
            }
            ...
            if ($couponDetails->minimumspent >  $total_amount) {
                $coupondata['err'] = 'This coupon is valid for minimun '.$couponDetails->minimumspent .'spent!';
                return $coupondata;
            }
